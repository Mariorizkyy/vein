...
import { Lock, PenTool, Calendar, ArrowLeft, CheckCircle2 } from "lucide-react";
...
export default function CreateVein() {
  ...
  const handleLock = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!unlockDate || !content.trim()) return;

    setIsLocking(true);
    setLockError(undefined);
    try {
      // Ritual Testnet: block.timestamp is in milliseconds, not seconds
      ...
      const hash = await writeContractAsync({
        ...
        // FIX: third param renamed 'content' in updated contract — still maps to same ABI slot
        args: [category, unlockTimestamp, content.trim()],
      });

      setTxHash(hash);
    } catch (err: any) {
      console.error(err);
      setLockError(err?.shortMessage ?? err?.message ?? "Transaction failed");
    } finally {
      setIsLocking(false);
    }
  };

  // Min datetime = now + 1 minute, so user can't accidentally pick past
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-[#FAFAF9] flex flex-col font-sans relative overflow-hidden">
      ...
      <header className="flex items-center justify-between px-8 py-6 w-full mx-auto border-b border-white/5 relative z-10 bg-black/20 backdrop-blur-md">
        ...
        <div className="font-extrabold text-lg tracking-[0.2em] text-transparent bg-clip-text bg-gradient-to-r from-white via-gray-400 to-white">
          V E I N
        </div>
      </header>
        ...
        <div className="glass-card rounded-3xl p-8 md:p-12 w-full max-w-2xl animate-fade-in-up">
          {!isConfirmed ? (
            <>
              ...
              <form onSubmit={handleLock} className="flex flex-col gap-8">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-medium text-[#A1A1AA] uppercase tracking-widest">
                    Category
                  </label>
                  <select
                    value={category}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <textarea
                    ...
                    required
                  />
                  <p className="text-[#A1A1AA] text-xs text-right">{content.length}/500</p>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <p className="text-[#A1A1AA] text-xs">
                    The AI will evaluate your prediction after this date.
                  </p>
                </div>
                ...
                <button
                  type="submit"
                  disabled={isLocking || isConfirming || !isConnected || !content.trim() || !unlockDate}
            ...
            <div className="text-center py-12 flex flex-col items-center animate-fade-in">
              ...
              <p className="text-[#A1A1AA] mb-2">Your belief has been sealed on-chain.</p>
              <p className="text-[#A1A1AA] text-sm mb-8">
                Return after the unlock date to trigger AI evaluation.
              </p>
